...
gcc zlib1g-dev wget make libcurl4-openssl-dev libbz2-dev liblzma-dev \
...
cd qctool && ./waf-1.5.18 configure--prefix=/usr/local/bin  && ./waf-1.5.18 install && \
hg clone https://bitbucket.org/gavinband/bgen && \
cd bgen && ./waf-1.8.13 configure --prefix=/usr/local/bin && ./waf-1.8.13 install
...
COPY --from=0 /qctool/build/release/qctool_v2.0-rc8 /usr/local/bin
COPY --from=0 /usr/local/bin/tabix /usr/local/bin && COPY --from=0 /usr/local/bin/bgzip /usr/local/bin/
COPY --from=0 /us/local/bin/htsfile /usr/local/bin
