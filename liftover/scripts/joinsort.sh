...
then
    ...
    | awk -v anew_chr=$anew_chr -v chr=$chr_as_is 'BEGIN{ FS="\t"; OFS="\t"} NR==1{ print $0,"REF","ALT"} NR>1{ split($1,a,":"); if(chr!="yes"){ gsub("chr", "", $anew_chr); gsub("X", "23", $anew_chr); gsub("Y", "24", $anew_chr); gsub("M", "25", $anew_chr) } print $0,a[3],a[4] } '| bgzip > $(basename $inputfile)".lifted.gz"
else
    anew_chr=$((cols+1))
